	...
	class apiparamtitle{
		...
		protected $_messages = array(
			'common' => array(
				...
				'example'	=>'@apiParam MyValidParamGroup Some title\n@apiParam (MyValidParamGroup) username',
			)
		);
		...
		//预处理		返回全局定义的内容
		public function preProcess($parsedFiles,$filenames,$packageInfos,$target='defineParamTitle'){
			$source = 'define';
			$result = array();
			$result[$target] = array();
		...
		//正式处理		定义的内容赋予继承者
		public function postProcess($parsedFiles,$filenames,$preProcess,$packageInfos,$source='defineParamTitle',$target='parameter',$messages=''){
			if(empty($messages))
