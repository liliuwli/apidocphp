<?php
	...
	class apiuse{
		...
		//预处理		返回全局定义的内容
		public function preProcess($parsedFiles,$filenames,$packageInfos,$target='define'){
			$source = $target;
			$result = array();
			$result[$target] = array();
		...
		//正式处理		定义的内容赋予继承者
		public function postProcess($parsedFiles,$filenames,$preProcess,$packageInfos,$source='define',$target='use',$messages=''){
			if(empty($messages))
