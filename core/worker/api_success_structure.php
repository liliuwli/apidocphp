<?php
	...
	class apisuccessstructure{
		...
		//预处理		返回全局定义的内容
		public function preProcess($parsedFiles,$filenames,$packageInfos,$target='defineSuccessStructure'){
			$source = $target;
			$result = array();
			$result[$target] = array();
		...
		//正式处理		定义的内容赋予继承者
		public function postProcess($parsedFiles,$filenames,$preProcess,$packageInfos,$source='defineSuccessStructure',$target='successStructure',$messages=''){
			if(empty($messages))
